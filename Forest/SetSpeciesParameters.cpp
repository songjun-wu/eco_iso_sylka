		...
		throw;
			...
			if(_tableParams->c !=42){
			 ...
			 for(UINT4 i = 0; i < _tableParams->r; i++){
				 ...
				 _species[i].SRA = _tableParams->matrix[i][17];
				 _species[i].Crown2Stem = _tableParams->matrix[i][18];
				 _species[i].TreeShapePar = _tableParams->matrix[i][19];
				 _species[i].WoodDensity = _tableParams->matrix[i][20];
				 _species[i].Fhdmax = _tableParams->matrix[i][21];
				 _species[i].Fhdmin = _tableParams->matrix[i][22];
				 _species[i].LeafTurnover = _tableParams->matrix[i][23];
				 _species[i].MaxLeafTurnoverWaterStress = _tableParams->matrix[i][24];; //maximum leaf turnover rate due to water stress (s-1)
				 _species[i].LeafTurnoverWaterStressShpParam = _tableParams->matrix[i][25];
				 _species[i].MaxLeafTurnoverColdStress = _tableParams->matrix[i][26];
				 _species[i].LeafTurnoverColdStressShpParam = _tableParams->matrix[i][27];
				 _species[i].ColdStressTemp = _tableParams->matrix[i][28];
				 _species[i].RootTurnover = _tableParams->matrix[i][29];
				 _species[i].MaxCanStorageParamt = _tableParams->matrix[i][30];
				 _species[i].albedo = _tableParams->matrix[i][31];
				 _species[i].emissivity = _tableParams->matrix[i][32];
				 _species[i].KBeers = _tableParams->matrix[i][33];
				 _species[i].beta = _tableParams->matrix[i][34];

				 _species[i].sperry_d = _tableParams->matrix[i][35];
				 _species[i].sperry_c = _tableParams->matrix[i][36];
				 _species[i].sperry_Kp = _tableParams->matrix[i][37];
				 _species[i].RAI_a = _tableParams->matrix[i][38];

				 //enter section for grasses if is_grass switch is on
				 _species[i].is_grass = _tableParams->matrix[i][39];
				 ...
				 if(_species[i].is_grass){
					 _species[i].Fprn = _tableParams->matrix[i][40]; //This reassigns dead grass leaf turnover rate to Fprn if is_grass=1.
					 _species[i].Fpra = _tableParams->matrix[i][41];//This reassign dead grass leaf turnover adjustment rate to reuse Fpra if is_grass=1
				 }
